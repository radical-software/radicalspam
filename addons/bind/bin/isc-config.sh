...
if test x"$echo_cflags" = x"true"; then
	...
	if test x"$libisc" = x"true"; then
		includes="$includes -D_REENTRANT  -I/addons/rs-tools/include  -D_GNU_SOURCE "
	fi
	echo $includes
fi
if test x"$echo_libs" = x"true"; then
	...
	if test x"$libdns" = x"true" ; then
		libs="$libs -ldns  -L/usr/local/src/rs-install/build/depends/lib -lcrypto"
	fi
	...
	if test x"$needothers" = x"true" ; then
		libs="$libs  -lpthread  -L/usr/local/src/rs-install/build/depends/lib -lxml2 -L/usr/local/src/rs-install/build/depends/lib -lz -L/usr/local/src/rs-install/build/depends/lib -liconv -lm"
	fi
	echo $libs
fi
